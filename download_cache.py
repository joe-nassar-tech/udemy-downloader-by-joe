...
class DownloadCache:
    ...
    def mark_download_failed(self, download_key, error_message=""):
        ...
        if download_key in self.cache_data["downloads"]:
            self.cache_data["downloads"][download_key].update({"status": "failed", "failed_at": datetime.now().isoformat(), "error": error_message})
    ...
    def get_failed_downloads(self):
        ...
        return [(key, data) for key, data in self.cache_data["downloads"].items() if data["status"] == "failed"]
