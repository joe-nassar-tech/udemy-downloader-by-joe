...
def download_and_merge_m3u8(m3u8_file_url, download_folder_path, title_of_output_mp4, task_id, progress, drm_key=None):
    ...
    if not highest_quality_url:
        progress.console.log(f"Invalid playlist URL for {remove_emojis_and_binary(title_of_output_mp4)} ✕")
        progress.remove_task(task_id)
        return
...
def merge_segments_into_mp4(m3u8_file_path, download_folder_path, output_file_name, task_id, progress, drm_key=None):
    ...
    # progress.console.log(f"[blue]Running command: {nm3u8dl_command}[/blue]")

    pattern = re.compile(r'(\d+\.\d+%)')
    process = subprocess.Popen(
        nm3u8dl_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='ignore'
    )
        ...
        while True:
            try:
                ...
                if output == '' and process.poll() is not None:
                    break
                if output:
                    # Use a more robust check for the output
                    if 'Vid' in output or '%' in output:
    ...
    if process.returncode != 0:
        progress.console.log(f"[red]Error Merging {remove_emojis_and_binary(output_file_name)} (Code {process.returncode})[/red] ✕")
        if stderr:
            progress.console.log(f"[red]Details: {stderr.strip()[:200]}[/red]")
        progress.remove_task(task_id)
        return
